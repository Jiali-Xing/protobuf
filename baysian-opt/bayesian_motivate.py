...
def run_experiments(interceptor_type, load, previous_run, **params):
    ...
    # if interceptor_type == 'dagorf':
    #     # search for latest file of dagor then rename it to dagorf
    #     latest_file = get_latest_file(os.path.expanduser('~/Sync/Git/protobuf/ghz-results/'), pattern=f"social-{method}-control-dagor-parallel-capacity-{load}-*.json")
    #     if latest_file is not None:
    #         # firstly, rename the output file to dagorf
    #         os.rename(latest_file + '.output', latest_file.replace('dagor', 'dagorf') + '.output')
    #         # rename the file to dagorf
    #         os.rename(latest_file, latest_file.replace('dagor', 'dagorf'))
    #         resultf = latest_file.replace('dagor', 'dagorf')
    # else:
...
def objective_dagorf(dagor_queuing_threshold, dagor_alpha, dagor_beta, dagor_admission_level_update_interval, dagor_umax):
    return objective_dual('dagorf', dagor_queuing_threshold=dagor_queuing_threshold, dagor_alpha=dagor_alpha, dagor_beta=dagor_beta, dagor_admission_level_update_interval=dagor_admission_level_update_interval, dagor_umax=dagor_umax)
